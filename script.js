...
function isTaskRunning(task) {
  return Boolean(task?.runningStartedAt && !task.completed);
}

function isTaskPaused(task) {
  return Boolean(!task?.completed && !task?.runningStartedAt && Number(task?.elapsedSeconds || 0) > 0);
}

function getTaskDurationSeconds(task) {
  return Math.max(0, Number(task?.minutes || 0) * 60);
}

function getTaskElapsedSeconds(task, now = new Date()) {
  const savedElapsed = Math.max(0, Number(task?.elapsedSeconds || 0));
  if (!task?.runningStartedAt) {
    return savedElapsed;
  }

  const startedAt = new Date(task.runningStartedAt);
  if (Number.isNaN(startedAt.getTime())) {
    return savedElapsed;
  }

  return savedElapsed + Math.max(0, Math.floor((now.getTime() - startedAt.getTime()) / 1000));
}

function getCountdownInfo(task, now = new Date()) {
  const duration = getTaskDurationSeconds(task);
  const elapsed = getTaskElapsedSeconds(task, now);
  return {
    duration,
    elapsed,
    remaining: Math.max(0, duration - elapsed),
    overdue: isTaskRunning(task) && elapsed > duration
  };
}
...
function render() {
  ...
  getScheduledTasks(getSortedTasks(getVisibleTasks())).forEach((task) => {
    ...
    item.className = `task-item${task.completed ? " completed" : ""}${task.hasConflict ? " conflict" : ""}${isTaskRunning(task) ? " running" : ""}${isTaskPaused(task) ? " paused" : ""}`;
    ...
    running.className = "task-running";
    running.textContent = isTaskRunning(task) ? "\u8fdb\u884c\u4e2d" : "\u5df2\u6682\u505c";
    ...
    main.append(priorityPill, time, start);
    if (isTaskRunning(task) || isTaskPaused(task)) {
      main.append(running);
    }
    main.append(text);
    if (task.hasConflict) {
    ...
    checkInButton.className = "check-in-task";
    checkInButton.textContent = isTaskRunning(task) ? "\u6682\u505c" : isTaskPaused(task) ? "\u7ee7\u7eed" : "\u6253\u5361";
    checkInButton.disabled = task.completed;
    checkInButton.addEventListener("click", () => checkInTask(task.id));
    ...
    buttons.append(checkInButton, editButton, deleteButton);
...
function toggleTask(id) {
  ...
  tasks = tasks.map((task) => {
    ...
    return {
      ...
      actualCompletedAt: completed ? nowIso : null,
      runningStartedAt: completed ? null : task.runningStartedAt,
      elapsedSeconds: completed ? getTaskElapsedSeconds(task, now) : Number(task.elapsedSeconds || 0)
    };
  });
...
function checkInTask(id) {
  ...
  const targetDate = normalizeTaskDate(target);
  const now = new Date();
  const nowIso = now.toISOString();
  const targetIsRunning = isTaskRunning(target);
  tasks = tasks.map((task) => {
    if (normalizeTaskDate(task) !== targetDate || task.completed) {
      return task;
    }
    ...
    if (task.id === id) {
      return {
        ...task,
        runningStartedAt: targetIsRunning ? null : nowIso,
        elapsedSeconds: targetIsRunning ? getTaskElapsedSeconds(task, now) : Number(task.elapsedSeconds || 0)
      };
    }
    ...
    if (isTaskRunning(task)) {
      return {
        ...task,
        runningStartedAt: null,
        elapsedSeconds: getTaskElapsedSeconds(task, now)
      };
    }
