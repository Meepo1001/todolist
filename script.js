...
const MINI_MODE_KEY = "todo2026.miniMode";
...
const miniModeButton = document.querySelector("#miniModeButton");
const miniPanel = document.querySelector("#miniPanel");
const expandModeButton = document.querySelector("#expandModeButton");
const miniStatus = document.querySelector("#miniStatus");
const miniTaskTitle = document.querySelector("#miniTaskTitle");
const miniTaskMeta = document.querySelector("#miniTaskMeta");
const miniProgressBar = document.querySelector("#miniProgressBar");
const miniCompleteButton = document.querySelector("#miniCompleteButton");
...
function getMiniTasks() {
  return getScheduledTasks(getSortedTasks(getVisibleTasks())).filter((task) => !task.completed);
}

function getMiniTaskInfo() {
  const miniTasks = getMiniTasks();
  if (!miniTasks.length) {
    return null;
  }
  ...
  const focused = miniTasks.find((task) => task.id === miniFocusTaskId);

  if (focused) {
    return { task: focused, status: "\u5df2\u9009\u4efb\u52a1", nowMinutes, selectedDate };
  }

  if (selectedDate === today) {
    const active = miniTasks.find((task) => {
      const start = timeToMinutes(task.startTime);
      const end = start + Number(task.minutes || 0);
      return nowMinutes >= start && nowMinutes < end;
    });
    if (active) {
      return { task: active, status: "\u6b63\u5728\u8fdb\u884c", nowMinutes, selectedDate };
    }

    const next = miniTasks.find((task) => timeToMinutes(task.startTime) >= nowMinutes);
    if (next) {
      return { task: next, status: "\u4e0b\u4e00\u4ef6\u4e8b", nowMinutes, selectedDate };
    }
  }

  return { task: miniTasks[0], status: "\u5f85\u529e\u4efb\u52a1", nowMinutes, selectedDate };
}

function formatTaskRange(task) {
  const start = task.startTime || dayStartTime;
  const end = minutesToTime(timeToMinutes(start) + Number(task.minutes || 0));
  return `${start} - ${end}`;
}

function getMiniProgress(info) {
  if (!info || info.selectedDate !== formatDate()) {
    return 0;
  }

  const start = timeToMinutes(info.task.startTime);
  const duration = Number(info.task.minutes || 0);
  if (!duration) {
    return 0;
  }

  return Math.max(0, Math.min(100, Math.round(((info.nowMinutes - start) / duration) * 100)));
}

function renderMiniPanel() {
  const info = getMiniTaskInfo();
  const pending = getMiniTasks().length;

  if (!info) {
    miniStatus.textContent = "\u5f53\u524d\u7a7a\u95f2";
    miniTaskTitle.textContent = "\u5f53\u524d\u6ca1\u6709\u672a\u5b8c\u6210\u4efb\u52a1";
    miniTaskMeta.textContent = `\u4eca\u5929\u5df2\u7ecf\u6ca1\u6709\u9700\u8981\u8ddf\u8fdb\u7684\u4efb\u52a1\u3002`;
    miniProgressBar.style.width = "0%";
    miniCompleteButton.disabled = true;
    miniNextButton.disabled = true;
    return;
  }

  const priority = priorityMap[info.task.priority] || priorityMap.medium;
  miniStatus.textContent = `${info.status} · ${priority.label}${label.important}`;
  miniTaskTitle.textContent = info.task.text;
  miniTaskMeta.textContent = `${formatTaskRange(info.task)} · ${info.task.minutes}${label.minute} · \u8fd8\u6709 ${pending} \u4e2a\u672a\u5b8c\u6210`;
  miniProgressBar.style.width = `${getMiniProgress(info)}%`;
  miniCompleteButton.disabled = false;
  miniNextButton.disabled = pending <= 1;
}
...
function completeMiniTask() {
  ...
  miniFocusTaskId = null;
  toggleTask(info.task.id);
}

function focusNextMiniTask() {
  const miniTasks = getMiniTasks();
  if (miniTasks.length <= 1) {
    return;
  }

  const currentId = getMiniTaskInfo()?.task.id;
  const currentIndex = miniTasks.findIndex((task) => task.id === currentId);
  const nextIndex = currentIndex >= 0 ? (currentIndex + 1) % miniTasks.length : 0;
  miniFocusTaskId = miniTasks[nextIndex].id;
  renderMiniPanel();
}
...
miniModeButton.addEventListener("click", () => setMiniMode(true));
expandModeButton.addEventListener("click", () => setMiniMode(false));
miniCompleteButton.addEventListener("click", completeMiniTask);
miniNextButton.addEventListener("click", focusNextMiniTask);
...
setMiniMode(localStorage.getItem(MINI_MODE_KEY) === "1");
miniRefreshTimer = window.setInterval(renderMiniPanel, 60 * 1000);
render();
