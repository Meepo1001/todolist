...
const label = {
  ...
  permissionNeeded: "\u9700\u8981\u91cd\u65b0\u6388\u6743\u624d\u80fd\u8bfb\u53d6\u8fd9\u4e2a\u6587\u4ef6\u5939\u3002",
...
function deferUnfinishedTasks() {
  ...
  const confirmed = confirm(`\u786e\u5b9a\u8981\u628a ${unfinished.length} \u4e2a\u672a\u5b8c\u6210\u4efb\u52a1\u987a\u5ef6\u5230 ${nextDate} \u5417\uff1f`);
  if (!confirmed) {
    return;
  }
...
function clearAll() {
  ...
  const confirmed = confirm("\u786e\u5b9a\u8981\u6e05\u7a7a\u5f53\u524d\u65e5\u671f\u7684\u6240\u6709\u4efb\u52a1\u5417\uff1f\u8fd9\u4f1a\u771f\u6b63\u5220\u9664\u8fd9\u4e00\u5929\u7684\u5f53\u524d\u4efb\u52a1\u548c\u5df2\u5f52\u6863\u5b8c\u6210\u8bb0\u5f55\u3002");
  if (!confirmed) {
    return;
  }
...
function showSummary() {
  ...
  if (typeof summaryDialog.showModal === "function") {
    ...
    alert(cells.map(([name, value]) => `${name}\uff1a${value}`).join("\n"));
  }
}
...
function exportExcel() {
  ...
  link.href = url;
  link.download = `${selectedDate}.xlsx`;
...
async function refreshCalendarData() {
  ...
  for await (const entry of directoryHandle.values()) {
    ...
    try {
      ...
      if (parsed) {
        days.push(...(Array.isArray(parsed) ? parsed : [parsed]));
        readFiles += 1;
      }
    } catch {
      failed += 1;
...
function renderDayDetail() {
  ...
  const items = day.tasks
    .map((task) => {
      ...
      return `<li>
        <span class="detail-pill" style="background:${priority.color}">${escapeHtml(task.priority)}</span>
        <span class="detail-time">${task.minutes}${label.minute}</span>
        <span class="detail-start">${escapeHtml(task.startTime || "")}</span>
        <span>${escapeHtml(task.text)}</span>
        <span class="detail-done">${task.completed ? label.yes : label.no}</span>
      </li>`;
    })
    .join("");

  dayDetail.innerHTML = `<h3>${escapeHtml(day.date)}</h3><ul>${items || `<li>${label.emptyDay}</li>`}</ul>`;
}
...
async function openMeeting() {
  ...
  if (!calendarDays.length && directoryHandle) {
    await refreshCalendarData();
  }
  renderMeetingGrid();
}
...
refreshCalendarButton.addEventListener("click", refreshCalendarData);
...
taskDateInput.max = formatDate(addDays(new Date(), 6));
